...
✅ present · ⚠️ derivable/partial · ❌ absent · ? unverified · INF = inference. Source: `docs/research/AGENT_STORAGE_FORMATS.md` *(internal)*.
...
4. **No cross-adapter coupling.** An adapter error affects only its source job. `catch_unwind` is a last-resort guard for unwind-safe code, **not a sandbox** and not protection from abort/OOM/deadlock; size/depth/time/output bounds (including the 64 MiB `MAX_SOURCE_BYTES` source file size cap that degrades oversized files to `partial`) and cancellable jobs provide the actual resource boundary.
