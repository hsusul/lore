...
- Keyset pagination on stable `(rank bucket, started_at, id)` or `(started_at,id)` cursors; no deep OFFSET pagination. **Implemented:** `search::search_page` orders by the total key `(bm25, started_at DESC, search_document.id)` and pages via an opaque cursor (`next_cursor`); a malformed cursor degrades to the first page. `search` is the first-page wrapper. The `started_at`-NULL block sorts last and has its own keyset case.
