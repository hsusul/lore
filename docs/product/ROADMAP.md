...
| M7 Safety and polish | In progress | Secret badges, redaction-aware export (with title newline, file path backtick, and nested code fence formatting sanitization), forget-session, forget-everything, privacy/settings surface, **user-visible backup cadence/settings and manual backup triggers** (`BackupSettings` UI + IPC schedule/restore wiring, `clamp` input bounding, `aria-busy` state signaling, aligned with `DEFAULT_BACKUP_RETENTION` of 7), **Lore-owned local backup create + restore + recovery wiring** (SQLite online backup into `backups/`, retention-pruned, `integrity_check`-verified, private perms; `restore_backup` + `list_backups` re-verify the destination without source logs; `recover_archive` quarantines a corrupt or truncated/0-byte archive under `quarantine/` — with cross-device `move_or_copy` fallback and best-effort sidecar quarantine protecting main archive recovery — and restores the newest backup, or reports the preserved quarantine path), **scanner-failure quarantine** (fallible `secrets::scan` — a scanner defect on untrusted input is captured, never a panic; the field is held out of search/export, no findings recorded, its recorded-patch blob is marked `failed_quarantined`, and `SecretScanState`/`SecretSeverity`/`SearchSourceKind`/`SearchSortOrder` are exported as typed TS unions), **boundary input validation & sanitization** (length bounds, limit clamping, canonical `is_zero_width` detection, Windows drive prefix neutralization, positive keyset rowid enforcement, control-character and zero-width path filtering across `sanitize_path` and `source_roots`, and control-character sanitization across all session, repository, search, backup schedule, setting, and folder IPC commands), **UI accessibility & keyboard polish** (roving `j`/`k`/`Home`/`End` navigation in search results listbox, pointer/disabled state protection on icon buttons, `aria-labelledby` modal labeling in Settings, `aria-busy` pagination feedback, live empty-result announcements in CommandPalette) | Complete deletion sweep, offline/egress and accessibility acceptance |
