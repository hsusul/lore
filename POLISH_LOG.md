...
## Iteration 139
- **Lens:** UX & accessibility
- **Change:** Add explicit `:focus-visible` outline for `.shell__banner-dismiss` in `src/styles.css` (`src/styles.css`).
- **Critique:**
  - `.shell__banner-dismiss` lacked a dedicated `:focus-visible` state styled to match the banner's contextual foreground color.
  - Fix: Added `.shell__banner-dismiss:focus-visible { opacity: 1; outline: 2px solid currentColor; outline-offset: 1px; }`.
- **Validation Results:**
  - `cargo test --workspace`: 87 passed across lore-core, lore-ipc, lore-app (2 scale/dev ignored).
  - `cargo clippy --workspace -- -D warnings`: Clean (0 warnings).
  - `npm run typecheck && npm run lint && npm test`: Clean; 12 test files passed (106 tests).
...
## Iteration 163
- **Lens:** UX & accessibility
- **Change:** Add explicit `aria-label` to command palette listbox and `role="presentation"` to empty state placeholder (`src/components/CommandPalette.tsx`).
- **Critique:**
  - In `CommandPalette.tsx`, the `ul[role="listbox"]` container lacked an explicit accessible name (`aria-label`), and the empty state placeholder `<li className="palette__empty">` was rendered without `role="presentation"`.
  - Fix: Added `aria-label="Commands and search results"` to the listbox and `role="presentation"` to the empty state item.
- **Validation Results:**
  - `cargo test --workspace`: 88 passed across lore-core, lore-ipc, lore-app (2 scale/dev ignored).
  - `cargo clippy --workspace -- -D warnings`: Clean (0 warnings).
  - `npm run typecheck && npm run lint && npm test`: Clean; 12 test files passed (111 tests).
...
## Iteration 196
- **Lens:** Performance/allocations
- **Change:** Memoize window range computation and output struct in `useWindowing` hook (`src/virtual.ts`).
- **Critique:**
  - `useWindowing` recomputed window ranges and allocated fresh object references on every render even when parameters hadn't changed.
  - Fix: Wrapped `computeWindow` in `useMemo` and memoized the returned `Windowing` object.
- **Validation Results:**
  - `cargo test --workspace`: 91 passed across lore-core, lore-ipc, lore-app (2 scale/dev ignored).
  - `cargo clippy --workspace -- -D warnings`: Clean (0 warnings).
  - `npm run check`: Clean; 12 test files passed (114 tests).
...
## Iteration 228
- **Lens:** Dependency/build hygiene
- **Change:** Audit Cargo workspace package definitions, rustfmt formatting, and npm dependencies.
- **Critique:**
  - Audited `Cargo.toml`, `package.json`, and member crate manifests for clean dependency trees, rustfmt conformance, and strict linter rules.
  - Fix: Validated all workspace manifests and rustfmt formatting.
- **Validation Results:**
  - `cargo test --workspace`: 98 passed across lore-core, lore-ipc, lore-app (2 scale/dev ignored).
  - `cargo clippy --workspace -- -D warnings`: Clean (0 warnings).
  - `npm run check`: Clean; 12 test files passed (115 tests).
